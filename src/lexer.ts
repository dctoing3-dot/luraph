// ============================================================================
// NEPHILIM OBFUSCATOR v1.0.0 - LURAPH STYLE EDITION (FULL)
// ============================================================================
...
// ============================================================================
// HELPERS - Luraph Style Generators
// ============================================================================
class NameGenerator{private chars='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';private counter=0;generate():string{return this.getName(this.counter++);}private getName(n:number):string{if(n<52)return this.chars[n];return this.getName(Math.floor(n/52)-1)+this.chars[n%52];}reset(){this.counter=0;}}
...
// ============================================================================
// PHASE 1: VARIABLE RENAMING (Short names: a, b, c...)
// ============================================================================
...
if(opts.renameVariables){renameMap=createRenameMap(tokens);tokens=applyRenameMap(tokens,renameMap);}
...
let decryptorName='';
let xorKey=0;
if(opts.encryptStrings){const encResult=encryptStrings(tokens);tokens=encResult.tokens;decryptorCode=encResult.encryption.decryptorCode;decryptorName=encResult.encryption.decryptorName;xorKey=encResult.encryption.xorKey;stringsEncrypted=encResult.encryption.encryptedStrings.size;}
...
if(opts.insertOpaquePredicates){const opaqueResult=injectOpaquePredicates(tokens,{insertionRate:opts.opaqueInsertionRate});tokens=opaqueResult.tokens;predicatesInserted=opaqueResult.predicatesInserted;deadCodeInserted=opaqueResult.deadCodeInserted;}

// Phase 7: Flatten Control Flow
let functionsFlattened=0;
if(opts.flattenControlFlow){const cffResult=flattenControlFlow(tokens,{flattenRate:opts.flattenRate,minStatements:3});tokens=cffResult.tokens;functionsFlattened=cffResult.functionsFlattened;}

// Phase 8: Function Proxy
let functionsProxied=0;
let proxyCode='';
if(opts.proxyFunctions){const proxyResult=createFunctionProxy(tokens);tokens=proxyResult.tokens;proxyCode=proxyResult.proxy.proxyCode;functionsProxied=proxyResult.proxy.functionsProxied;}

// Generate Inner Code
let innerCode=tokensToCode(tokens);

// Wrap in Luraph Style
