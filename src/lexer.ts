// ============================================================================
// NEPHILIM OBFUSCATOR v0.8.1 - PHASE 7 & 8: FIXED SPACING
// ============================================================================
...
export function injectOpaquePredicates(tokens:Token[],config:{insertionRate?:number}={}):{tokens:Token[];predicatesInserted:number;deadCodeInserted:number}{const cfg={insertionRate:config.insertionRate??0.1};const opaqueGen=new OpaquePredicateGenerator();const deadGen=new DeadCodeGenerator();const result:Token[]=[];let predicatesInserted=0;let deadCodeInserted=0;const insertionPoints:number[]=[];for(let i=0;i<tokens.length;i++){const t=tokens[i];if(t.type===TokenType.THEN||t.type===TokenType.DO||t.type===TokenType.ELSE){if(Math.random()<cfg.insertionRate){insertionPoints.push(i);}}}for(let i=0;i<tokens.length;i++){const t=tokens[i];result.push(t);if(insertionPoints.includes(i)){const insertDeadCode=Math.random()<0.5;if(insertDeadCode){const falsePred=opaqueGen.generateFalsePredicate();const deadCode=deadGen.generateDeadBlock();const injection=`${falsePred.setup} if ${falsePred.condition} then ${deadCode} end`;const injTokens=tokenize(injection);injTokens.pop();result.push(...injTokens);predicatesInserted++;deadCodeInserted++;}else{const truePred=opaqueGen.generateTruePredicate();const injTokens=tokenize(truePred.setup);injTokens.pop();result.push(...injTokens);predicatesInserted++;}}}return{tokens:result,predicatesInserted,deadCodeInserted};}
interface CFBlock{id:number;tokens:Token[];nextState:number|null}
class ControlFlowFlattener{private stateVarName:string;private blocks:CFBlock[]=[];constructor(){this.stateVarName=this.generateStateVarName();}private generateStateVarName():string{const chars=['I','l'];let name='_';for(let i=0;i<6;i++){name+=chars[Math.floor(Math.random()*2)];}return name;}private shuffleArray<T>(array:T[]):T[]{const result=[...array];for(let i=result.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[result[i],result[j]]=[result[j],result[i]];}return result;}flattenFunction(bodyTokens:Token[]):Token[]{this.blocks=[];const segments=this.splitIntoSegments(bodyTokens);if(segments.length<2)return bodyTokens;let stateId=1;for(const seg of segments){this.blocks.push({id:stateId,tokens:seg,nextState:stateId<segments.length?stateId+1:0});stateId++;}return this.generateStateMachine();}private splitIntoSegments(tokens:Token[]):Token[][]{const segments:Token[][]=[];let current:Token[]=[];let depth=0;for(let i=0;i<tokens.length;i++){const t=tokens[i];if(t.type===TokenType.IF||t.type===TokenType.WHILE||t.type===TokenType.FOR||t.type===TokenType.FUNCTION||t.type===TokenType.REPEAT||t.type===TokenType.DO)depth++;if(t.type===TokenType.END||t.type===TokenType.UNTIL)depth--;current.push(t);if(depth===0&&current.length>3&&(t.type===TokenType.END||i===tokens.length-1||(tokens[i+1]&&tokens[i+1].type===TokenType.LOCAL))){segments.push(current);current=[];}}if(current.length>0)segments.push(current);return segments;}private generateStateMachine():Token[]{const result:Token[]=[];const shuffled=this.shuffleArray(this.blocks);const init=this.blocks.length>0?this.blocks[0].id:0;result.push(...tokenize(`local ${this.stateVarName}=${init}`).slice(0,-1));result.push(...tokenize(`while ${this.stateVarName}~=0 do `).slice(0,-1));for(let i=0;i<shuffled.length;i++){const block=shuffled[i];const kw=i===0?'if':'elseif';result.push(...tokenize(`${kw} ${this.stateVarName}==${block.id} then `).slice(0,-1));result.push(...block.tokens);result.push(...tokenize(` ${this.stateVarName}=${block.nextState??0} `).slice(0,-1));}result.push(...tokenize(` end end`).slice(0,-1));return result;}}
export function flattenControlFlow(tokens:Token[],config:{flattenRate?:number;minStatements?:number}={}):{tokens:Token[];functionsFlattened:number}{const cfg={flattenRate:config.flattenRate??0.3,minStatements:config.minStatements??3};const result:Token[]=[];let functionsFlattened=0;let i=0;while(i<tokens.length){const t=tokens[i];if(t.type===TokenType.FUNCTION){result.push(t);i++;while(i<tokens.length&&tokens[i].type!==TokenType.LPAREN){result.push(tokens[i]);i++;}if(i<tokens.length){result.push(tokens[i]);i++;}let parenDepth=1;while(i<tokens.length&&parenDepth>0){if(tokens[i].type===TokenType.LPAREN)parenDepth++;if(tokens[i].type===TokenType.RPAREN)parenDepth--;result.push(tokens[i]);i++;}const bodyTokens:Token[]=[];let depth=1;while(i<tokens.length&&depth>0){const tk=tokens[i];if(tk.type===TokenType.FUNCTION||tk.type===TokenType.IF||tk.type===TokenType.WHILE||tk.type===TokenType.FOR||tk.type===TokenType.REPEAT||tk.type===TokenType.DO)depth++;if(tk.type===TokenType.END||tk.type===TokenType.UNTIL){depth--;if(depth===0)break;}bodyTokens.push(tk);i++;}if(bodyTokens.length>=cfg.minStatements&&Math.random()<cfg.flattenRate){const flattener=new ControlFlowFlattener();result.push(...flattener.flattenFunction(bodyTokens));functionsFlattened++;}else{result.push(...bodyTokens);}if(i<tokens.length){result.push(tokens[i]);i++;}}else{result.push(t);i++;}}return{tokens:result,functionsFlattened};}
// ============================================================================
// PHASE 7: FUNCTION PROXYING + GARBAGE CODE (FIXED)
// ============================================================================
...
class GarbageFunctionGenerator{
    ...
    generateGarbageFunction():string{
        const name=this.generateName();
        // FIXED: Menghindari {} dan {...} langsung
        const strategies=[
            ()=>`local function ${name}(a,b) return a and b or a end`,
            ()=>`local function ${name}(x) if x then return x*2 else return 0 end end`,
            ()=>`local function ${name}(s) return s and #s or 0 end`,
            ()=>`local function ${name}(a,b,c) return (a or 0)+(b or 0)+(c or 0) end`,
            ()=>`local function ${name}(t,k) return t and t[k] or nil end`,
            ()=>`local function ${name}(n) local r=0 for i=1,n or 0 do r=r+i end return r end`,
            ()=>`local function ${name}(a,b) if a and b then return a+b end return 0 end`,
            ()=>`local function ${name}(x) return x and tostring(x) or "" end`
        ];
    ...
    generateMultiple(count:number):string{
        ...
        for(let i=0;i<count;i++){
            funcs.push(this.generateGarbageFunction());
        }
        return funcs.join(' ');
    }
}
...
export function createFunctionProxy(tokens:Token[]):{tokens:Token[];proxy:FunctionProxyResult}{
    const proxyTableName=generateObfuscatedName('_P');
    const usedFunctions=new Set<string>();
    
    for(const t of tokens){
        if(t.type===TokenType.IDENTIFIER&&PROXYABLE_FUNCTIONS.includes(t.value)){
            usedFunctions.add(t.value);
        }
    }
    
    if(usedFunctions.size===0){
        return{tokens,proxy:{proxyTableName,proxyCode:'',functionsProxied:0}};
    }
    ...
    // FIXED: Menggunakan format yang aman dengan spacing
    const proxyCode=`local ${proxyTableName}={}`;
    const proxyInit=entries.map(e=>`${proxyTableName}.${e}`).join(' ');
    const fullProxyCode=`${proxyCode} ${proxyInit}`;
    
    const result:Token[]=[];
    for(let i=0;i<tokens.length;i++){
        const t=tokens[i];
        if(t.type===TokenType.IDENTIFIER&&proxyMap[t.value]){
            const prev=tokens[i-1];
            if(prev&&(prev.type===TokenType.DOT||prev.type===TokenType.COLON)){
                result.push(t);
                continue;
            }
            result.push({type:TokenType.IDENTIFIER,value:proxyTableName,line:t.line,column:t.column});
            result.push({type:TokenType.DOT,value:'.',line:t.line,column:t.column});
            result.push({type:TokenType.IDENTIFIER,value:proxyMap[t.value],line:t.line,column:t.column});
        }else{
            result.push(t);
        }
    }
    
    return{tokens:result,proxy:{proxyTableName,proxyCode:fullProxyCode,functionsProxied:usedFunctions.size}};
}

export function injectGarbageFunctions(count:number):string{
    const generator=new GarbageFunctionGenerator();
...
export interface AntiDebugConfig{
    ...
    crashOnDetect:boolean
}

const DEFAULT_ANTI_DEBUG_CONFIG:AntiDebugConfig={
    checkDebugLibrary:true,
    checkHooks:true,
...
export function generateAntiDebugCode(config:Partial<AntiDebugConfig>={}):string{
    const cfg={...DEFAULT_ANTI_DEBUG_CONFIG,...config};
    const checks:string[]=[];
    const varPrefix=generateObfuscatedName('_AD');
    const flagVar=`${varPrefix}_f`;
    const checksVar=`${varPrefix}_c`;
    ...
    if(cfg.checkIntegrity){
        const checkName=`${varPrefix}_i`;
        const magicNum=Math.floor(Math.random()*9000)+1000;
        checks.push(`local function ${checkName}() local n=${magicNum} local m=n*2-n if m~=${magicNum} then return true end return false end`);
        checks.push(`${checksVar}[#${checksVar}+1]=${checkName}`);
    }
    
    const runChecks=`${varPrefix}_run`;
...
export function generateAntiTamperCode():string{
    const varPrefix=generateObfuscatedName('_AT');
    const hashVar=`${varPrefix}_h`;
    const checkVar=`${varPrefix}_c`;
    const code:string[]=[];
    
    code.push(`local ${hashVar}=0`);
...
export function generateRuntimeProtection():string{
...
// MAIN OBFUSCATE
// ============================================================================
export interface ObfuscateOptions{debug?:boolean;renameVariables?:boolean;encryptStrings?:boolean;obfuscateNumbers?:boolean;insertOpaquePredicates?:boolean;opaqueInsertionRate?:number;obfuscateProperties?:boolean;obfuscateTableKeys?:boolean;flattenControlFlow?:boolean;flattenRate?:number;proxyFunctions?:boolean;garbageFunctions?:number;antiDebug?:boolean;antiTamper?:boolean;runtimeProtection?:boolean}
export interface ObfuscateResult{code:string;map:RenameMap;stats:{originalTokens:number;identifiersRenamed:number;stringsEncrypted:number;numbersObfuscated:number;predicatesInserted:number;deadCodeInserted:number;propertiesObfuscated:number;tableKeysObfuscated:number;functionsFlattened:number;functionsProxied:number;garbageFunctionsAdded:number;originalLength:number;outputLength:number;timeMs:number};debugLogs?:DebugLog[]}
export function obfuscate(source:string,options:ObfuscateOptions={}):ObfuscateResult{const startTime=Date.now();const opts={debug:options.debug??false,renameVariables:options.renameVariables??true,encryptStrings:options.encryptStrings??true,obfuscateNumbers:options.obfuscateNumbers??true,insertOpaquePredicates:options.insertOpaquePredicates??true,opaqueInsertionRate:options.opaqueInsertionRate??0.1,obfuscateProperties:options.obfuscateProperties??true,obfuscateTableKeys:options.obfuscateTableKeys??true,flattenControlFlow:options.flattenControlFlow??true,flattenRate:options.flattenRate??0.3,proxyFunctions:options.proxyFunctions??true,garbageFunctions:options.garbageFunctions??5,antiDebug:options.antiDebug??true,antiTamper:options.antiTamper??true,runtimeProtection:options.runtimeProtection??false};enableDebug(opts.debug);debug('MAIN','Nephilim v0.8.1');let tokens=tokenize(source);const originalTokenCount=tokens.length;let renameMap:RenameMap={};if(opts.renameVariables){renameMap=createRenameMap(tokens);tokens=applyRenameMap(tokens,renameMap);}let stringsEncrypted=0;let prependCode='';let decryptorName='';let xorKey=0;if(opts.encryptStrings){const encResult=encryptStrings(tokens);tokens=encResult.tokens;prependCode=encResult.encryption.decryptorCode;decryptorName=encResult.encryption.decryptorName;xorKey=encResult.encryption.xorKey;stringsEncrypted=encResult.encryption.encryptedStrings.size;}let propertiesObfuscated=0;if(opts.obfuscateProperties&&decryptorName){const propResult=obfuscatePropertyAccess(tokens,decryptorName,xorKey);tokens=propResult.tokens;propertiesObfuscated=propResult.propertiesObfuscated;}let tableKeysObfuscated=0;if(opts.obfuscateTableKeys&&decryptorName){const keyResult=obfuscateTableKeys(tokens,decryptorName,xorKey);tokens=keyResult.tokens;tableKeysObfuscated=keyResult.keysObfuscated;}let numbersObfuscated=0;if(opts.obfuscateNumbers){const numResult=obfuscateNumbers(tokens);tokens=numResult.tokens;numbersObfuscated=numResult.numbersObfuscated;}let predicatesInserted=0;let deadCodeInserted=0;if(opts.insertOpaquePredicates){const opaqueResult=injectOpaquePredicates(tokens,{insertionRate:opts.opaqueInsertionRate});tokens=opaqueResult.tokens;predicatesInserted=opaqueResult.predicatesInserted;deadCodeInserted=opaqueResult.deadCodeInserted;}let functionsFlattened=0;if(opts.flattenControlFlow){const cffResult=flattenControlFlow(tokens,{flattenRate:opts.flattenRate,minStatements:3});tokens=cffResult.tokens;functionsFlattened=cffResult.functionsFlattened;}let functionsProxied=0;if(opts.proxyFunctions){const proxyResult=createFunctionProxy(tokens);tokens=proxyResult.tokens;if(proxyResult.proxy.proxyCode){prependCode=prependCode+' '+proxyResult.proxy.proxyCode;}functionsProxied=proxyResult.proxy.functionsProxied;}let garbageFunctionsAdded=0;if(opts.garbageFunctions&&opts.garbageFunctions>0){const garbageCode=injectGarbageFunctions(opts.garbageFunctions);prependCode=prependCode+' '+garbageCode;garbageFunctionsAdded=opts.garbageFunctions;}if(opts.antiDebug){const antiDebugCode=generateAntiDebugCode();prependCode=antiDebugCode+' '+prependCode;}if(opts.antiTamper){const antiTamperCode=generateAntiTamperCode();prependCode=antiTamperCode+' '+prependCode;}if(opts.runtimeProtection){const runtimeCode=generateRuntimeProtection();prependCode=runtimeCode+' '+prependCode;}const code=tokensToCode(tokens,prependCode);return{code,map:renameMap,stats:{originalTokens:originalTokenCount,identifiersRenamed:Object.keys(renameMap).length,stringsEncrypted,numbersObfuscated,predicatesInserted,deadCodeInserted,propertiesObfuscated,tableKeysObfuscated,functionsFlattened,functionsProxied,garbageFunctionsAdded,originalLength:source.length,outputLength:code.length,timeMs:Date.now()-startTime},debugLogs:opts.debug?getDebugLogs():undefined};}
